...
# THIS MODEL USES A DYNAMIC NN2 to TRAIN NN1
...
DUAL2_LOAD_PATH = './dual_kinematic_nn2.pth'
DUAL2_SAVE_PATH = './dual_kinematic_nn2.pth'
...
learning_rate = 0.01
momentum = 0
optimizer1 = optim.SGD(net1.parameters(), learning_rate, momentum,\
                       dampening=0, weight_decay=0, nesterov=False)
optimizer2 = optim.SGD(net2.parameters(), learning_rate, momentum,\
                       dampening=0, weight_decay=0, nesterov=False)
...
if (yesno('Train neural net')):
    for epoch in range(N_epochs):  # loop over the dataset multiple times
        ...
        for i in range(N_batch):
            ...
            # note: this time, we will allow NN2 weights to change

            # NN2: backward + optimize + loss
    ...
    q_batch_list = torch.split(qt, 1, dim=2)

    # merge batches into a single column of (1x2) rows of (x1,x2)
    x_stacked = torch.cat(x_batch_list, dim=0)
    q_stacked = torch.cat(q_batch_list, dim=0)

    # eliminate the empty batch dimension (dim=2)
    x_stacked = torch.squeeze(x_stacked,dim=2)
    q_stacked = torch.squeeze(q_stacked,dim=2)

    print('reshape 3D batched data -> 2D unbatched...')
    print('    batched x size                : ', xt.size())
    print('    unbatched/stacked x size      : ', x_stacked.size())
    print('    batched q size                : ', qt.size())
    print('    unbatched/stacked output size : ', q_stacked.size())
    ...
    if (torch.equal(x_stacked[0:batch_size,:], torch.squeeze(xt[:,:,0]))):
        print('x reshape: VALID. (i.e. consistent up to first batch) :)\n')
    else:
        print('WARNING!! Invalid Reshape (x).  Good f**king luck.\n')
        
    if (torch.equal(q_stacked[0:batch_size,:], torch.squeeze(qt[:,:,0]))):
        print('q reshape: VALID. (i.e. consistent up to first batch) :)\n')
    else:
        print('WARNING!! Invalid Reshape (q).  Good f**king luck.\n')
    ...
    MAE = torch.sub(out_nn2, x_stacked)

    # convert to numpy
    x_stacked_np = x_stacked.detach().numpy()
    q_stacked_np = q_stacked.detach().numpy()
    out_nn1_np = out_nn1.detach().numpy()
    out_nn2_np = out_nn2.detach().numpy()

    # plot predicted vs. actual: N/N-2
    ...
    # plot predicted vs. actual: N/N-1
    fig, ax = plt.subplots(2, 1, sharex=True, sharey=True)
    ax[0].scatter(q_stacked_np[:,0], out_nn1_np[:,0], 1, label='q_1')
    ax[0].set_title("q_1")
    ax[0].set(ylabel='predicted')

    ax[1].scatter(q_stacked_np[:,1], out_nn1_np[:,1], 1, label='q_2')
    ax[1].set_title("q_2")
    ax[1].set(xlabel='true', ylabel='predicted')
    fig.tight_layout()
    plt.show()

    # plot error for both training and validation: N/N-1
    plt.figure
    plt.plot(range(bcount), mean_loss, label='Training')
    plt.plot(range(bcount), mean_vloss, label='Validation')
    plt.title('N/N-1 Neural Net Training with Coupled Dynamic N/N-2')
    ...
    if (yesno('Save neural net')):
        torch.save(net1.state_dict(), DUAL2_SAVE_PATH)
        
else:
    net = Net()
    net.load_state_dict(torch.load(DUAL2_LOAD_PATH))
